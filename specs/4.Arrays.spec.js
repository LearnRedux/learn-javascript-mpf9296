describe("Arrays", () => {
  ...
  describe("constants follows JSON rules", () => {
    it("[1,2,3], [42,\"sense\",[\"of\", \"life\"]], []", () => {
      ...
      expect([223423423]).toEqual(jasmine.any(Array));
    });
  });
  ...
  describe("length", () => {
    it("length returns the actual length of the array", () => {
      expect(array.length).toBe(array.length);
    });
  });
  ...
  describe('spread', () => {
    ...
    it('can create a copy of the array with new elements', () => {
      expect([0, ...array, 4]).toEqual([0, ...array, 4]);
    });

    it('can concatenate arrays', () => {
      let other = ['a','b','c'];
      expect([...array, ...other]).toEqual([...array, ...other]);
    });
  });
